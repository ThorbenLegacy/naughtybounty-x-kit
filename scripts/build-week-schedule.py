...
def main() -> None:
    ...
    if target:
        ...
        # Nur den aktuellsten Wochenplan bauen (höchstes weekStart-Datum)
        all_weeks = sorted(WEEKS.glob("*.json"))
        if not all_weeks:
            print("Keine Wochenpläne in config/weeks/")
            sys.exit(1)
        latest = max(all_weeks, key=lambda p: json.loads(p.read_text(encoding="utf-8")).get("weekStart", ""))
        files = [latest]
        print(f"Aktuellster Plan: {latest.name}")
